...
export default function GameId() {
    const { id } = useLocalSearchParams<GameParams>();
        ...
        return (
            <Container style={{flexDirection: "column", justifyContent: "center"}}>
                <Card style={{alignItems: 'center', margin: 20}} variant="elevated">
                    <Text style={{fontSize: 24, fontWeight: 'bold', marginBottom: 16}}>🎉 Partie terminée !</Text>
                    {ranking.length > 0 && (
                        <Text style={{fontSize: 18, marginBottom: 20}}>Gagnant : {ranking[0].getName()}</Text>
                    )}
                    <Button
            ...
            >
                {gameStatus === 'started' && (
                    <ThemedView style={{alignItems: 'center', flex: 1, width: "100%"}}>
                        ...
                        {!canPlay && (
                            <>
                                <Card style={{margin: 16, alignItems: 'center'}} variant="elevated">
                                    <Text style={{fontSize: 24, textAlign: 'center', marginBottom: 12, fontWeight: 'bold'}}>
                                    ...
                                    {!hasAgainDart && currentPlayerInRow && (
                                        <Text style={{
                                            ...
                                            {currentPlayerInRow.getDarts().reduce((a, b) => a + (b.score * b.multiplier), 0)} points
                                        </Text>
                                    )}
                                    {hasOverScore && (
                                        <Text style={{
                        ...
                        <View style={styles.resumeDarts}>
                            {currentPlayerInRow?.getDarts().map((dart, index) => (
                                <View key={index} style={styles.relative}>
                                    ...
                                    {index === (currentPlayerInRow?.getDarts().length ?? 0) - 1 && (
                                        <TabBarIcon
                                            name="trash"
                ...
                <Modal
                    ...
                    size="md"
                >
                    <Title style={{alignSelf: 'center', marginBottom: 20}}>Paramètres</Title>
                ...
                <Modal
                    ...
                    size="md"
                >
                    <Title style={{alignSelf: 'center', marginBottom: 20}}>🏆 Classement</Title>
                    
                    <View style={{gap: 12}}>
                        {[...players].sort((a, b) => a.getScore() - b.getScore()).map((player, index) => {
                            const isLeader = index === 0;
                            return (
                                <Card key={player.getId()} variant={isLeader ? "elevated" : "outlined"}>
                                    <View style={{
                                        flexDirection: 'row',
                                        alignItems: 'center',
                                        ...
                                        <View style={{flexDirection: 'row', alignItems: 'center'}}>
                                            <Text style={{
                                                ...
                                                fontWeight: 'bold',
                                                marginRight: 12,
                                                color: isLeader ? '#f59e0b' : (theme === 'dark' ? '#ffffff' : '#374151')
                                            }}>
                                                {isLeader ? '👑' : `${index + 1}.`}
                                            </Text>
                                            <Text style={{
                                        ...
                                        <Text style={{
                                            fontSize: 16,
                                            fontWeight: 'bold',
                                            color: player.getScore() === 0 ? '#10b981' : (theme === 'dark' ? '#ffffff' : '#374151')
                                        }}>
                                            {player.getScore()} pts
                                        </Text>
                                    </View>
