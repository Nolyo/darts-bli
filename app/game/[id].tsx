...
export default function GameId() {
  ...
  const [showLastRoundPanel, setShowLastRoundPanel] = useState<boolean>(false);
  const [confirmLeaveVisible, setConfirmLeaveVisible] =
  ...
  // Interception retour arrière: confirmer si partie non finie
  useEffect(() => {
    const unsubscribe = navigation.addListener("beforeRemove", (e: any) => {
      if (gameStatus === "finished") return; // pas de confirmation si fini
      const actionType = e?.data?.action?.type;
      // Confirme uniquement pour les retours (POP/GO_BACK)
        ...
        >
          {gameStatus === "started" && (
            ...
            >
              ...
              {/* Bandeau repliable: Historique du dernier tour */}
              {(() => {
                const totalPlayers = players.length;
                const currentIdx = Math.max(0, (gameRows.length || 1) - 1);
                const currentRow = gameRows[currentIdx] || [];
                const isCurrentRowComplete =
                  currentRow.length === totalPlayers && totalPlayers > 0;
                const lastCompletedIdx = isCurrentRowComplete
                  ? currentIdx
                  : Math.max(0, currentIdx - 1);
                const hasAnyCompleted =
                  (gameRows[lastCompletedIdx]?.length || 0) === totalPlayers &&
                  totalPlayers > 0;
                if (!hasAnyCompleted) return null;

                const lastRound = gameRows[lastCompletedIdx];
                return (
                  <Card
                    variant="outlined"
                    style={{
                  ...
                  >
                    <Pressable
                      onPress={() => setShowLastRoundPanel((v) => !v)}
                      style={{
                        flexDirection: "row",
                        alignItems: "center",
                        justifyContent: "space-between",
                    ...
                    >
                      <Text style={{ fontWeight: "700", fontSize: 16 }}>
                        Dernier tour ({lastCompletedIdx + 1})
                      </Text>
                    ...
                    {showLastRoundPanel && (
                      <View style={{ marginTop: 10, gap: 8 }}>
                        {lastRound.map((pir, idx) => (
                          <View
                            key={`${pir.getPlayer().getId?.() ?? idx}`}
                            style={{
                              flexDirection: "row",
                              alignItems: "center",
                              justifyContent: "space-between",
                          ...
                          >
                            <Text style={{ fontWeight: "600" }}>
                              {pir.getPlayer().getName()}
                            </Text>
                            <View style={{ flexDirection: "row", gap: 8 }}>
                              {pir.getDarts().map((d, i) => (
                                <View
                                  key={i}
                                  style={{
                                    backgroundColor:
          ...
          {/* Confirmation de sortie si partie non finie */}
          <Modal
          ...
          >
            ...
            <Text style={{ textAlign: "center", marginBottom: 16 }}>
              La partie n’est pas terminée. Voulez-vous revenir à l’accueil ?
            </Text>
